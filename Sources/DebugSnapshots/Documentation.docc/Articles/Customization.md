...
cases are not omitted entirely, just their associated values.
...
The `isLoading` property will now be included in any snapshot taken, allowing you to debug this
state easily, as well as exhaustively test logic involving it when using the library's testing
tools.

You can also track private, encapsulated properties without broadening their access control:
